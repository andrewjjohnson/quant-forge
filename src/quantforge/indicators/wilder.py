...
class WilderDirectionalMovement:
    ...
    def __init__(
        self,
        parameters: WilderDirectionalMovementParameters,
        *,
        backend_id: str | None = None,
        backend_registry: IndicatorBackendRegistry | None = None,
    ) -> None:
    ...
    @property
    def standard_definition(self) -> StandardIndicatorDefinition:
        """Return the canonical directional-movement definition for every backend."""
        return self._definition
    ...
    def from_configuration(
        ...
        *,
        ...
        """Restore legacy/native or explicit backend semantics without drift."""
        period = _serialized_period(configuration, "directional movement")
        backend_id = _serialized_backend_id(configuration, "directional movement")
        indicator = cls(
            WilderDirectionalMovementParameters(period),
            backend_id=backend_id,
            backend_registry=backend_registry,
        )
        if indicator.configuration() != configuration:
            raise IndicatorBackendVersionError(
                "serialized directional-movement configuration does not match the "
                "installed backend"
            )
        return indicator
