...
def captured_study(
    ...
    completed = complete_study(tmp_path, prediction=bool(request.param))
...
def test_rehashed_fold_state_must_match_the_complete_captured_record(
    tmp_path: Path, captured_study: CompletedStudy, field: str
) -> None:
    source = captured_study.source
    path = (
        captured_study.study.study_path
        / "folds"
        / source.folds[0].fold_id
        / "state.json"
    )
    ...
    with pytest.raises(ManifestError, match="fold state differs from captured source"):
        inspect_validation(
            source, captured_study.study.study_path, artifact_root=tmp_path
        )


def incomplete_source(completed: CompletedStudy) -> tuple[OOSSource, Path, Path]:
...
def test_unchanged_failed_and_missing_folds_remain_indexable(
    tmp_path: Path, captured_study: CompletedStudy
) -> None:
...
def test_pending_state_presence_cannot_change_after_source_capture(
    tmp_path: Path, captured_study: CompletedStudy, delete: bool
) -> None:
    ...
    write_record(path, state)
    if delete:
        source = load_oos_source(source.plan, captured_study.study.study_path)
