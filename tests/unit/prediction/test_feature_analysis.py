...
def test_analysis_requires_false_as_the_unavailable_availability_default(
    ...
    unavailable_values = configured_outcome["unavailable_values"]
    assert isinstance(unavailable_values, dict)
    unavailable_values["available"] = True
    ...
    with pytest.raises(
        SignalFeatureDatasetError,
        match="unavailable default of false",
    ):
        analyze_signal_features(
            invalid_dataset,
            feature_names=(feature_name,),
